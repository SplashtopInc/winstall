...
见 proposal.md 的 Why。`AddAppsDialog` 默认打全量 `GET /apps` 分页替换列表；`AddAppPickerCard` 是旧皮。`/category` 已用 `CategoryTabs` + `fetchCategoryApps` + Load more（`limit` 56）+ compact `SingleApp`。弹窗有独立本地选择集与 Already added，不能并入 `SelectedContext`。归档后本文件归属 `openspec/specs/pack-add-apps/design.md`。
...
**备选：** 搜索时隐藏下拉——出口只剩清空输入。搜索时灰掉下拉——摩擦更大。分类 ∩ 关键词——要新 API，本 change 不做。真正把 `activeSlug` 改成 `all`——清空搜索无法回到原分类。
...
只发 Web。回滚：恢复 `AddAppsDialog` 全量分页与旧 picker 皮，去掉嵌入的分类 tabs。
