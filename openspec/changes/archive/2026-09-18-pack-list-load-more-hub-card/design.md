...
见 proposal.md 的 Why。`pages/packs/index.js` 无 `tab` 时 `activeTab` 为 `mine`；Public 用 `PACKS_PER_PAGE = 24` 整页替换并画 Prev/Next。`PackCard` 固定 `height: 250px`，中间最多 6 个 app 图标。归档后本文件归属 `openspec/specs/pack-list-browse/design.md`。
...
**理由：** 上次把 `q`/`tab` 当可分享路由已否决。`/apps?q=` 才是目录级搜索。
...
**理由：** 用户选「时间 + 三连都留」。相对时间对齐参考图，三连保持站内互动语言。
