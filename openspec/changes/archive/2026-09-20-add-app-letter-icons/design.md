...
见 proposal.md 的 Why。`components/AppIcon.js` 是全站应用图标入口：本地精选 / `localIconOverrides` → 成对 `iconUrl`+`iconPng` → 空 `icon` 走 `GenericAppIcon`（`/generic-app-icon.svg`）→ 外链或 CDN 文件名；`ExternalAppIcon` 与 `AppPicture` 的 `onError` 也落到包装盒。`name` 已在绝大多数调用点传入但未使用。`PackPreview` 用 `a.icon != ""` 丢掉空字段应用。归档后本文件归属 `openspec/specs/app-letter-icons/design.md`。
